...
# HIDDEN_MODE kontrolü - bash penceresini gizlemek için
if [ "$HIDDEN_MODE" = "1" ]; then
    # Windows/Cygwin için bash penceresini gizle
    if [ "$OSTYPE" = "cygwin" ] || [ "$UNAME" = "CYGWIN" ]; then
        # Sadece stdin'i kapat, stdout/stderr'i koru (log için)
        exec < /dev/null
        
        # Pencereyi gizlemek için terminal escape kodları
        printf '\033[8;1;1t' 2>/dev/null || true  # Terminal boyutunu 1x1 yap
        printf '\033[2t' 2>/dev/null || true      # Pencereyi minimize et
        
        # Terminal title'ı gizle
        printf '\033]0;\007' 2>/dev/null || true
    fi
fi
...
DNSCHECK_DOM=${DNSCHECK_DOM:-roblox.com ntc.party rutracker.org www.torproject.org bbc.com}
...
exitp()
{
	...
	# HIDDEN_MODE veya BATCH modunda read komutunu atla
	[ "$HIDDEN_MODE" = "1" ] || [ "$BATCH" = 1 ] || {
		echo
...
ask_params()
{
	...
	# Auto-configure parameters for silent execution
